...
import me.ka1.vulcan.util.RotationUtil;
...
public class ShulkerAura extends Module {
    ...
    public void setup() {
        ...
        detonateDelay = registerInteger("Detonate Delay","Detonate Delay", 4, 1, 10);
        endDelay = registerInteger("Await Delay","Await Delay", 4, 1, 10);
        restartDelay = registerInteger("Attempt Delay","Attempt Delay", 4, 1, 10);
    }
    ...
    private EntityPlayer getTarget(){
        ...
        for(EntityPlayer player : mc.world.playerEntities){
            if(player != null && player != mc.player && player.getHealth()>0 && mc.player.getDistance(player)<5){
                if(!Vulcan.getInstance().friends.isFriend(player.getName())){
                    temp = player;
                }
            }
        }

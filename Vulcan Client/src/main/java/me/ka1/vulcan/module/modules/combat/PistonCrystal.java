...
import me.ka1.vulcan.event.events.OnUpdateWalkingPlayerEvent;
...
import static me.ka1.vulcan.util.SpoofRotationUtil.ROTATION_UTIL;

//This is why i dont use open source clients - 2q1

public class PistonCrystal extends Module {

    Setting.Double enemyRange;
    Setting.Double crystalDeltaBreak;
    Setting.Integer crystalPlaceTry;
    Setting.Integer blocksPerTick;
    Setting.Integer minHealth;
    Setting.Integer supBlocksDelay;
    Setting.Integer preRotationDelay;
    Setting.Integer afterRotationDelay;
    Setting.Integer startDelay;
    Setting.Integer pistonDelay;
    Setting.Integer crystalDelay;
    Setting.Integer redstoneDelay;
    ...
    Setting.Boolean packetReducer;
    Setting.Boolean preRotation;
    Setting.Boolean forceRotation;
    ...
    public void setup() {
        ...
        modes.add("Observer");
        modes.add("None");
        placeMode = registerMode("PlaceMode", "placeMode", modes, modes.get(1));

        packetReducer = registerBoolean("packetReducer", "packetReducer", true);
        preRotation = registerBoolean("preRotation", "preRotation", true);
        forceRotation = registerBoolean("forceRotation", "forcerotation", true);
        chatMsg = registerBoolean("chatMsg", "chatmsg", true);
        enemyRange = registerDouble("Range", "Range", 4.9, 0, 6);
        crystalDeltaBreak = registerDouble("Center Break","Center Break", 0.1, 0, 0.5);
        crystalPlaceTry = registerInteger("Crystal Place Try","Crystal Place Try", 15, 2, 30);
        blocksPerTick = registerInteger("Blocks Per Tick","Blocks Per Tick", 4, 0, 20);
        minHealth = registerInteger("Min Health","Min Health", 8, 0, 20);
        supBlocksDelay = registerInteger("Surround Delay","Surround Delay", 4, 0, 20);
        preRotationDelay = registerInteger("Pre Rotation Delay","Pre Rotation Delay", 0, 0, 20);
        afterRotationDelay = registerInteger("After Rotation Delay","After Rotation Delay", 0, 0, 20);
        startDelay = registerInteger("Start Delay","Start Delay", 4, 0, 20);
        pistonDelay = registerInteger("Piston Delay","Piston Delay", 2, 0, 20);
        crystalDelay = registerInteger("Crystal Delay","Crystal Delay", 2, 0, 20);
        redstoneDelay = registerInteger("Redstone Delay","Redstone Delay", 0, 0, 20);
        midHitDelay = registerInteger("Mid Hit Delay","Mid Hit Delay", 5, 0, 20);
        hitDelay = registerInteger("Hit Delay","Hit Delay", 2, 0, 20);
        stuckDetector = registerInteger("Stuck Check","Stuck Check", 35, 0, 200);
        maxYincr = registerInteger("Max Y","Max Y", 3, 0, 5);
        blockPlayer = registerBoolean("Trap Player","Trap Player", true);
        rotate = registerBoolean("Rotate","Rotate", false);
        allowCheapMode = registerBoolean("Cheap Mode","Cheap Mode", false);
        betterPlacement = registerBoolean("Better Place","Better Place", true);
        bypassObsidian = registerBoolean("Bypass Obsidian","Bypass Obsidian", false);
    ...
    public PistonCrystal() {
        ...
        public void onDisable() {
            ...
            if (!materialsNeeded.equals(""))
                Command.sendRawMessage("Materials missing:" + materialsNeeded);
        ...
        private final Listener<OnUpdateWalkingPlayerEvent> onUpdateWalkingPlayerEventListener = new Listener<>(event -> {
            if (event.getEra() != MinecraftEvent.Era.PRE || !rotate.getValue() || lastHitVec == null || !forceRotation.getValue()) return;
                             ...
                             */
                                 ...
                                 */
                                    ...
                                    if (someoneInCoords(torchCoords[0], torchCoords[2])
                                            ...
                                            || BlockUtil.getBlock(torchCoords[0], torchCoords[1], torchCoords[2]) instanceof BlockObserver)
		 ...
		 */
            ...
            if (placeMode.getValue().equals("Block") || placeMode.getValue().equals("Observer"))
                ...
                if (stack.getItem() instanceof ItemBlock) {
                    ...
                    if (block instanceof BlockObsidian) {
                        ...
                        if (block instanceof BlockPistonBase) {
                            ...
                            //I am i wasnt born here, i speak BRIISH TEA MATE INNIT U WANNA GET SHEFFED FAM
                                // UR MUM BARE BUTTERS INNIT G

                            // RedstoneTorch / RedstoneBlock
                            if (placeMode.getValue().equals("None") && block instanceof BlockRedstoneTorch) {
                                slot_mat[3] = i;
                                redstoneBlockMode = false;
                            } else if (placeMode.getValue().equals("Block") && block.translationKey.equals("blockRedstone")) {
                                slot_mat[3] = i;
                                redstoneBlockMode = true;
                            }else if (placeMode.getValue().equals("Observer") && block instanceof BlockObserver) {
                                slot_mat[3] = i;
                                redstoneBlockMode = true;
                            }
...
//Fucking 1629 lines of this shite
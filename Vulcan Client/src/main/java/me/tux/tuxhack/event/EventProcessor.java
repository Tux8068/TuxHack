...
import me.tux.tuxhack.event.events.PlayerJoinEvent;
import me.tux.tuxhack.event.events.PlayerLeaveEvent;
...
public class EventProcessor {
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState()) {
            ...
            //Macro
            TuxHack.getInstance().macroManager.getMacros().forEach(m -> {
    ...
    @SubscribeEvent
    public void onRenderBlockOverlay(RenderBlockOverlayEvent event){ TuxHack.EVENT_BUS.post(event); }
                        ...
                        new Thread(() -> {
                            ...
                            if (name != null) {
                                if (mc.player != null && mc.player.ticksExisted >= 1000)
                                    TuxHack.EVENT_BUS.post(new PlayerJoinEvent(name));
                            }
                        }).start();
                    }
                }
                ...
                for (SPacketPlayerListItem.AddPlayerData playerData : packet.getEntries()) {
                    if (playerData.getProfile().getId() != mc.session.getProfile().getId()) {
                        new Thread(() -> {
                            ...
                            if (name != null) {
                                if (mc.player != null && mc.player.ticksExisted >= 1000)
                                    TuxHack.EVENT_BUS.post(new PlayerLeaveEvent(name));
                            }
                        }).start();
                    }
                }
